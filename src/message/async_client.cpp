 ...
 *
 *
 * Version: $Id: async_client.cpp 49 2010-11-16 09:58:57Z zongdai@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace message {
        ...
        AsyncClient::AsyncClient(const VUINT64& ds, AsyncCallback* cb)
                :
                handle_response_count_(0), handle_success_count_(0), send_success_count_(0), call_over_(false),
                send_message_(
                        NULL), callback_(cb)
        {
            assert(ds.size());
        ...
        {
            if (message!=NULL) {
                send_message_ = ClientManager::gClientManager.factory_.clone_message(const_cast<Message*>(message), -1,
                        true);
        ...
        int32_t AsyncClient::post(const Message* msg)
        {
            ...
            for (i = 0; i<list_size; ++i) {
                ...
                Message* send_msg = ClientManager::gClientManager.factory_.clone_message(const_cast<Message*> (msg), 2,
                        false);
                if (send_msg==NULL) {
                ...
                mutex_.lock();
                if (!ClientManager::gClientManager.connmgr_->sendPacket(dsip, send_msg, NULL,
                        (void*) (static_cast<long>(call_id_)))) {
                    TBSYS_LOG(ERROR, "client(%d) send message cid(%d) to server(%s) failure, pcode:%d", call_id_,
                            send_msg->getChannelId(), tbsys::CNetUtil::addrToString(dsip).c_str(),
                            send_msg->getPCode());
                    delete send_msg;
                    call_over_ = true;
                    mutex_.unlock();
        ...
        int AsyncClient::handlePacket(tbnet::Packet* packet, void*)
        {
            ...
            if (!packet->isRegularPacket()) { // failed
            ...
            if (message->get_message_type()==STATUS_MESSAGE) {
                ...
                else { // ok, check all server return..
