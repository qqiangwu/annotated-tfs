 ...
 *
 *
 * Version: $Id: message.h 42 2010-11-12 07:32:15Z nayan@taobao.com $
 *
 * Authors:
...
#pragma pack()
        ...
        public:
            Message()
                    :
                    raw_data_(NULL), raw_size_(0), connection_(NULL), direction_(DIRECTION_SEND), id_(0), crc_(0),
                    version_(2),
                    auto_free_(true)
            {
            }
            ...
            {
                if (getChannelId()==0) {
                    TBSYS_LOG(ERROR, "message (%d) channel is null, reply message (%d)", getPCode(),
                            message->getPCode());
                    delete message;
                    return;
                }
                if (((direction_ & DIRECTION_RECEIVE) && _expireTime>0) && (tbsys::CTimeUtil::getTime()>_expireTime)) {
                    TBSYS_LOG(ERROR, "message (%d), timeout for response, reply message (%d)", getPCode(),
                            message->getPCode());
                    delete message;
                    return;
                }
