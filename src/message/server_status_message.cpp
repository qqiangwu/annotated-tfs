 ...
 *
 *
 * Version: $Id: server_status_message.cpp 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace message {
        ...
        SetServerStatusMessage::SetServerStatusMessage()
                :
                Message(),
                status_type_(0), from_row_(0), return_row_(1000), has_next_(0), need_free_(0),
        ...
        int SetServerStatusMessage::parse(char* data, int32_t len)
        {
            ...
            if ((status_type_>0) && (status_type_ & SSM_SERVER)) {
                ...
                for (i = 0; i<size; i++) {
                    ...
                    if (get_object_copy(&data, &len, reinterpret_cast<void*>(ds), sizeof(DataServerStatInfo))
                            ==TFS_ERROR) {
                        delete server_collect;
                        return TFS_ERROR;
                    }
                    server_map_.insert(SERVER_MAP::value_type(ds->id_, server_collect));
                    get_int_map(&data, &len,
                            const_cast<std::set<uint32_t>*> (server_collect->get_writable_block_list()));
                    get_int_map(&data, &len,
                            const_cast<std::set<uint32_t>*>(server_collect->get_primary_writable_block_list()));
                }
            }
        ...
        int SetServerStatusMessage::build(char* data, int32_t len)
        {
            ...
            if ((status_type_>0)
                ...
                for (i = 0; i<chunk_num; ++i) {
                    ...
                    for (; it!=current.end(); ++it) {
                        ...
                        if (count>from_row_+return_row_)
                            break;
                        if (set_object(&data, &len, const_cast<BlockInfo*>(it->second->get_block_info()),
                                BLOCKINFO_SIZE)==TFS_ERROR) {
                            return TFS_ERROR;
                        }
            ...
            if ((status_type_>0)
                ...
                for (SERVER_MAP_ITER it = server_map_.begin(); it!=server_map_.end(); it++) {
                    ...
                    if (set_int_map(&data, &len, const_cast<set <uint32_t>*>(servcol->get_writable_block_list()), wsize)
                            ==TFS_ERROR) {
                        return TFS_ERROR;
                    }
                    if (set_int_map(&data, &len,
                            const_cast<set <uint32_t>*>(servcol->get_primary_writable_block_list()),
                            servcol->get_primary_writable_block_list()->size())==TFS_ERROR) {
                        return TFS_ERROR;
                    }
                }
            }
        ...
        int ReplicateInfoMessage::build(char* data, int32_t len)
        {
            ...
            for (REPL_BLOCK_MAP::const_iterator it = replicating_map_.begin(); it!=replicating_map_.end(); ++it) {
                ...
                if (set_object(&data, &len, reinterpret_cast<void*> (const_cast<ReplBlock*>(&it->second)),
                        sizeof(ReplBlock))==TFS_ERROR) {
                    return TFS_ERROR;
                }
            }
                ...
                int32_t return_row, int32_t size)
        {
            ...
            for (; it!=map.end(); ++it) {
                ...
                if (set_object(data, len, reinterpret_cast<void*> (const_cast<Throughput*>(&it->second)),
                        sizeof(Throughput))==TFS_ERROR) {
                    return TFS_ERROR;
                }
            }
            return TFS_SUCCESS;
        ...
        GetBlockListMessage::GetBlockListMessage()
                :
                request_flag_(1), write_flag_(0), start_block_id_(0), start_inclusive_(0), end_block_id_(0),
                read_count_(0),
        ...
        int GetBlockListMessage::build(char* data, int32_t len)
        {
            ...
            while (index<chunk_num) {
                ...
                while (it!=current.end()) {
                    ...
                    chunk_done = false;
                    if (set_object(&data, &len,
                            reinterpret_cast<void*>(const_cast<BlockInfo*>(it->second->get_block_info())),
                            BLOCKINFO_SIZE)==TFS_ERROR) {
                        return TFS_ERROR;
                    }
                    if (set_vint64(&data, &len, const_cast<VUINT64&>(*it->second->get_ds()))==TFS_ERROR) {
