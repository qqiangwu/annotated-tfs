...
namespace tfs {
    namespace dataserver {
        ...
        // ------------------------------------------------------------------------------------------
        int
        IndexHandle::create(const uint32_t logic_block_id, const int32_t cfg_bucket_size, const MMapOption map_option,
        ...
        int IndexHandle::load(const uint32_t logic_block_id, const int32_t cfg_bucket_size, const MMapOption map_option)
        {
            ...
            if (file_size<index_file_size) {
                TBSYS_LOG(ERROR,
                        "Index corrupt error. blockid: %u, bucket size: %d, file size: %d, index file size: %d",
            ...
            if (logic_block_id!=block_info()->block_id_) {
                TBSYS_LOG(ERROR, "block id conflict. blockid: %u, index blockid: %u", logic_block_id,
            ...
            TBSYS_LOG(
                    ...
                    index_header()->free_head_offset_, block_info()->seq_no_, block_info()->size_,
        ...
        int IndexHandle::remove(const uint32_t logic_block_id)
        {
            if (is_load_) {
                if (logic_block_id!=block_info()->block_id_) {
                    TBSYS_LOG(ERROR, "block id conflict. blockid: %d, index blockid: %d", logic_block_id,
                            block_info()->block_id_);
                    return EXIT_BLOCKID_CONFLICT_ERROR;
                }
            }
        ...
        int IndexHandle::find_avail_key(uint64_t& key)
        {
            ...
            if (!found) {
                TBSYS_LOG(ERROR, "blockid: %u, find avail key fail. new key: %" PRI64_PREFIX "u",
        ...
        {
            TBSYS_LOG(DEBUG, "block version. blockid: %u, remote version: %u, local version: %u",
                    block_info()->block_id_,
        ...
        int IndexHandle::delete_segment_meta(const uint64_t key)
        {
            ...
            else // delete from list, modify previous
            {
                ...
                ret = file_op_->pwrite_file(reinterpret_cast<const char*> (&pre_meta_info), META_INFO_SIZE,
                        previous_offset);
                if (TFS_SUCCESS!=ret)
                    return ret;
            }
        ...
        int IndexHandle::hash_insert(const int32_t slot, const int32_t previous_offset, const RawMeta& meta)
        {
            ...
            if (0!=previous_offset) {
                ...
                ret = file_op_->pwrite_file(reinterpret_cast<const char*> (&tmp_meta_info), META_INFO_SIZE,
                        previous_offset);
                if (TFS_SUCCESS!=ret)
                    return ret;
            }
