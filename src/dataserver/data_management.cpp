 ...
 *
 *
 * Version: $Id: data_management.cpp 40 2010-11-09 03:01:34Z zongdai@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace dataserver {
        ...
        void
        DataManagement::get_ds_filesystem_info(int32_t& block_count, int64_t& use_capacity, int64_t& total_capacity)
        {
        ...
        int DataManagement::create_file(const uint32_t block_id, uint64_t& file_id, uint64_t& file_number)
        {
            ...
            if (0==file_id) {
                ...
                if (TFS_SUCCESS!=ret) {
                    TBSYS_LOG(ERROR, "try getfileid failed. blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d.",
                            block_id,
                            file_id, ret);
                    return ret;
                }
                ...
                const char* data_buffer, UpdateBlockType& repair)
        {
            ...
            if (NULL==datafile) {
                TBSYS_LOG(ERROR,
                        "datafile is null. blockid: %u, fileid: %" PRI64_PREFIX "u, filenumber: %" PRI64_PREFIX "u",
            ...
            if (write_len!=write_info.length_) {
                TBSYS_LOG(
                        ...
                        write_info.block_id_, write_info.file_id_, write_info.file_number_, write_info.length_,
                        write_len);
                // clean dirty data
        ...
        int DataManagement::close_write_file(const CloseFileInfo& closefileInfo, int32_t& write_file_size)
        {
            ...
            if (TIMER_DURATION()>SYSPARAM_DATASERVER.max_io_warn_time_) {
                TBSYS_LOG(WARN,
                        "write file cost time: blockid: %u, fileid: %" PRI64_PREFIX "u, cost time: %" PRI64_PREFIX "d",
                ...
                int32_t& real_read_len, char* tmp_data_buffer)
        {
            ...
            if (end-start>SYSPARAM_DATASERVER.max_io_warn_time_) {
                TBSYS_LOG(WARN,
                        "read file cost time: blockid: %u, fileid: %" PRI64_PREFIX "u, cost time: %" PRI64_PREFIX "d",
                ...
                char* tmp_data_buffer)
        {
            ...
            if (TFS_SUCCESS!=ret) {
                TBSYS_LOG(ERROR, "blockid: %u read data batch error, offset: %d, rlen: %d, ret: %d", block_id,
                        read_offset,
                        real_read_len, ret);
                return ret;
            }
        ...
        int DataManagement::rename_file(const uint32_t block_id, const uint64_t file_id, const uint64_t new_file_id)
        {
            ...
            if (file_id==new_file_id) {
                TBSYS_LOG(WARN,
                        "rename file fail. blockid:%u, fileid: %" PRI64_PREFIX "u, newfileid: %" PRI64_PREFIX "u",
        ...
        int DataManagement::unlink_file(const uint32_t block_id, const uint64_t file_id, const int32_t action)
        {
            ...
            if (TFS_SUCCESS!=ret) {
                TBSYS_LOG(ERROR, "del file fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d", block_id, file_id,
                        ret);
                return ret;
            }
                ...
                int32_t& set_count)
        {
            ...
            if (NORMAL_BIT_MAP==query_type) {
                BlockFileManager::get_instance()->query_bit_map(tmp_data_buffer, bit_map_len, set_count,
                        C_ALLOCATE_BLOCK);
            }
            else {
        ...
        int
        DataManagement::query_block_status(const int32_t query_type, VUINT& block_ids, std::map<uint32_t, std::vector<
        ...
        int DataManagement::add_new_expire_block(const VUINT32* expire_block_ids, const VUINT32* remove_block_ids,
