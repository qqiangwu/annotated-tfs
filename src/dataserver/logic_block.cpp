 ...
 *
 *
 * Version: $Id: logic_block.cpp 33 2010-11-01 05:24:35Z nayan@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace dataserver {
        ...
        int
        LogicBlock::init_block_file(const int32_t bucket_size, const MMapOption mmap_option, const BlockType block_type)
        {
        ...
        int LogicBlock::close_write_file(const uint64_t inner_file_id, DataFile* datafile, const uint32_t crc)
        {
            ...
            if (TFS_SUCCESS==ret) {
                TBSYS_LOG(INFO, "file exist, update! blockid: %u, fileid: %" PRI64_PREFIX "u", logic_block_id_,
                        inner_file_id);
            }
            ...
            if (file_meta.get_file_id()==inner_file_id) {
                ...
                if (TFS_SUCCESS!=ret) {
                    TBSYS_LOG(ERROR, "read FileInfo fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d",
                            logic_block_id_,
                            inner_file_id, ret);
                }
            ...
            do {
                ...
                while ((tmp_data_buffer = datafile->get_data(NULL, &read_len, read_offset))!=NULL) {
                    if (read_len<0 || (read_len+read_offset)>file_size) {
                        TBSYS_LOG(ERROR,
                                "getdata fail, blockid: %u, fileid: %" PRI64_PREFIX "u, size: %d, offset: %d, rlen: %d",
                    ...
                    if (0==read_offset) {
                        ...
                        ret = data_handle_->write_segment_data(tmp_write_buffer, write_len,
                                file_meta.get_offset()+write_offset);
                        delete[] tmp_write_buffer;
                    }
                    else                  // write real file data
                    {
                        ret = data_handle_->write_segment_data(tmp_data_buffer, write_len,
                                file_meta.get_offset()+write_offset);
                    }
        ...
        int LogicBlock::read_file(const uint64_t inner_file_id, char* buf, int32_t& nbytes, const int32_t offset)
        {
            ...
            int ret = index_handle_->read_segment_meta(inner_file_id, file_meta);
            if (TFS_SUCCESS!=ret) {
                TBSYS_LOG(ERROR, "blockid: %u read file meta fail, fileid: %" PRI64_PREFIX "u, ret: %d",
                        logic_block_id_,
                        inner_file_id, ret);
                return ret;
            }
            ...
            if (TFS_SUCCESS!=ret) {
                TBSYS_LOG(ERROR,
                        "blockid: %u read data error, fileid: %" PRI64_PREFIX "u, size: %d, offset: %d, ret: %d",
        ...
        int LogicBlock::read_file_info(const uint64_t inner_file_id, FileInfo& finfo)
        {
            ...
            TBSYS_LOG(
                    ...
                    finfo.id_, finfo.offset_, finfo.size_, finfo.usize_, finfo.modify_time_, finfo.create_time_,
                    finfo.flag_,
                    finfo.crc_);
        ...
        int LogicBlock::unlink_file(const uint64_t inner_file_id, const int32_t action)
        {
            ...
            int ret = index_handle_->read_segment_meta(inner_file_id, file_meta);
            if (TFS_SUCCESS!=ret) {
                TBSYS_LOG(ERROR, "read segment meta fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d",
                        logic_block_id_,
                        inner_file_id, ret);
                return ret;
            }
            ...
            case REVEAL:
                ...
                break;
            default:
                TBSYS_LOG(ERROR, "action is illegal. action: %d, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d",
                        action,
            ...
            if (TFS_SUCCESS!=ret) {
                TBSYS_LOG(ERROR, "update block info fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d",
                        logic_block_id_,
                        inner_file_id, ret);
                return ret;
            }
            ...
            while (retry_times) {
                if (offset+size>avail_data_size_) // need extend block
                {
                    TBSYS_LOG(INFO,
                            ...
                            logic_block_id_, offset, size, avail_data_size_, index_handle_->get_block_data_offset(),
                            retry_times);
        ...
        int FileIterator::next()
        {
            ...
            if (cur_fileinfo_.id_!=meta_it_->get_file_id() || cur_fileinfo_.size_!=file_size) {
                TBSYS_LOG(
                        ...
                        logic_block_->logic_block_id_, cur_fileinfo_.id_, cur_fileinfo_.size_, meta_it_->get_file_id(),
                        file_size);
            }
