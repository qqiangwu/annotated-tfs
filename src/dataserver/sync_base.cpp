 ...
 *
 *
 * Version: $Id: sync_base.cpp 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace dataserver {
        ...
        int SyncBase::write_sync_log(const int32_t cmd, const uint32_t block_id, const uint64_t file_id,
                const uint64_t old_file_id)
        {
            if (stop_ || 0==need_sync_) {
                TBSYS_LOG(INFO, " no need write sync log, blockid: %u, fileid: %" PRI64_PREFIX "u, need sync: %d",
                        block_id,
                        file_id, need_sync_);
                return TFS_SUCCESS;
            }
        ...
        int SyncBase::do_sync(const char* data, const int32_t len, const bool second)
        {
            ...
            if (TFS_SUCCESS!=ret) {
                TBSYS_LOG(WARN,
                        "sync error! cmd: %d, blockid: %u, fileid: %" PRI64_PREFIX "u, old fileid: %" PRI64_PREFIX "u, ret: %d, sf->retry_count: %d",
