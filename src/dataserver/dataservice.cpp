 ...
 *
 *
 * Version: $Id: dataservice.cpp 51 2010-11-22 05:30:16Z zongdai@taobao.com $
 *
 * Authors:
    ...
    namespace dataserver {
            ...
            if (connection!=NULL && connection->getLocalPort()==server_local_port_+1) {
                ...
                if (!access_deny(bp)) {
                    ...
                    MessageFactory::send_error_message(bp, TBSYS_LOG_LEVEL(DEBUG), STATUS_MESSAGE_ACCESS_DENIED,
                            data_server_info_.id_, "you client %s access been denied. msgtype: %d",
        ...
        int DataService::create_file_number(CreateFilenameMessage* message)
        {
            ...
            if (TFS_SUCCESS!=ret) {
                ...
                MessageFactory::send_error_message(message, TBSYS_LOG_LEVEL(ERROR), data_server_info_.id_,
                        "create file failed. blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d.", block_id, file_id,
                        ret);
            }
            else {
        ...
        int DataService::write_data(WriteDataMessage* message)
        {
            ...
            TBSYS_LOG(
                    ...
                    write_info.block_id_, write_info.file_id_, write_info.file_number_, version, lease_id,
                    write_info.is_server_);
            ...
            if (EXIT_NO_LOGICBLOCK_ERROR==ret) {
                ...
                if (TFS_SUCCESS!=ds_requester_.req_update_block_info(write_info.block_id_, repair)) {
                    TBSYS_LOG(ERROR, "req update block info failed. blockid: %u, repair: %d", write_info.block_id_,
                            repair);
                }
            }
            ...
            TBSYS_LOG(
                    ...
                    ret>=0 ? "success" : "fail", write_info.file_number_, write_info.block_id_, write_info.file_id_,
                    version, lease_id, Master_Server_Role==write_info.is_server_ ? "master" : "slave",
                    TIMER_DURATION());
            return TFS_SUCCESS;
        }
        ...
        int DataService::close_write_file(CloseFileMessage* message)
        {
                ...
                if (TFS_SUCCESS!=ret) {
                    ...
                    if (CLOSE_FILE_SLAVER!=close_file_info.mode_) {
                        ...
                        if (TFS_SUCCESS!=ret) {
                            ...
                            //commit
                            ret = ds_requester_.req_block_write_complete(close_file_info.block_id_, lease_id,
                                    TFS_SUCCESS);
                            if (TFS_SUCCESS==ret) {
                                //sync to mirror
                                int option_flag = message->get_option_flag();
                                if (0==(option_flag & TFS_FILE_NO_SYNC_LOG)) {
                                    TBSYS_LOG(INFO, " write sync log, blockid: %u, fileid: %" PRI64_PREFIX "u",
            ...
            TIMER_END();
            TBSYS_LOG(INFO,
                    "close file %s. filenumber: %" PRI64_PREFIX "u, blockid: %u, fileid: %" PRI64_PREFIX "u, peerip: %s, role: %s, cost time: %" PRI64_PREFIX "d",
        ...
        int DataService::read_data_v2(ReadDataMessageV2* message)
        {
            ...
            TBSYS_LOG(DEBUG, "blockid: %u, fileid: %" PRI64_PREFIX "u, read len: %d, read offset: %d, resp: %p",
                    block_id,
            ...
            TIMER_END();
            TBSYS_LOG(INFO,
                    "read v2 %s. blockid: %u, fileid: %" PRI64_PREFIX "u, read len: %d, read offset: %d, peer ip: %s, cost time: %" PRI64_PREFIX "d",
        ...
        int DataService::read_data(ReadDataMessage* message)
        {
            ...
            TIMER_END();
            TBSYS_LOG(INFO,
                    "read %s. blockid: %u, fileid: %" PRI64_PREFIX "u, read len: %d, read offset: %d, peer ip: %s, cost time: %" PRI64_PREFIX "d",
                    TFS_SUCCESS==ret ? "success" : "fail", block_id, file_id, real_read_len, read_offset,
                    tbsys::CNetUtil::addrToString(peer_id).c_str(), TIMER_DURATION());
            return ret;
        }
        ...
        int DataService::read_raw_data(ReadRawDataMessage* message)
        {
            ...
            if (0!=real_read_len) {
                if (NULL==packet_data) {
                    ...
                    tbsys::gDeleteA(tmp_data_buffer);
                    TBSYS_LOG(ERROR, "allocdata fail, blockid: %u, realreadlen: %" PRI64_PREFIX "d", block_id,
                            real_read_len);
                    return TFS_ERROR;
                }
                else {
        ...
        int DataService::read_file_info(FileInfoMessage* message)
        {
            ...
            TIMER_END();
            TBSYS_LOG(DEBUG,
                    "read fileinfo %s. blockid: %u, fileid: %" PRI64_PREFIX "u, mode: %d, cost time: %" PRI64_PREFIX "d",
        ...
        int DataService::unlink_file(UnlinkFileMessage* message)
        {
            ...
            if (TFS_SUCCESS!=ret) {
                if (EXIT_NO_LOGICBLOCK_ERROR==ret) {
                    ...
                    try_add_repair_task(block_id, ret);
                    MessageFactory::send_error_message(message, TBSYS_LOG_LEVEL(WARN), data_server_info_.id_,
                            "file unlink fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d", block_id, file_id,
                            ret);
                }
            }
            else {
                ...
                if (is_master && 0==(option_flag & TFS_FILE_NO_SYNC_LOG)) {
                    TBSYS_LOG(DEBUG,
                            "master dataserver: delete synclog. blockid: %d, fileid: %" PRI64_PREFIX "u, action: %d\n",
            ...
            TIMER_END();
            TBSYS_LOG(INFO,
                    "unlink file %s. blockid: %d, fileid: %" PRI64_PREFIX "u, action: %d, isserver: %s, peer ip: %s, cost time: %" PRI64_PREFIX "d",
        ...
        int DataService::new_block(NewBlockMessage* message)
        {
            ...
            if (TFS_SUCCESS!=ret) {
                return MessageFactory::send_error_message(message, TBSYS_LOG_LEVEL(DEBUG), data_server_info_.id_,
        ...
        int DataService::replicate_block_cmd(ReplicateBlockMessage* message)
        {
            ...
            TBSYS_LOG(
                    ...
                    b->block_id_, tbsys::CNetUtil::addrToString(b->source_id_).c_str(), tbsys::CNetUtil::addrToString(
                            b->destination_id_).c_str(), b->server_count_,
                    tbsys::CNetUtil::addrToString(peer_id).c_str());
        ...
        int DataService::compact_block_cmd(CompactBlockMessage* message)
        {
            ...
            TBSYS_LOG(INFO, "receive compact cmd. blockid: %u, owner: %d, preserve_time: %d, haverb: %d, peer_id: %s\n",
                    cblk->block_id_, cblk->owner_, cblk->preserve_time_, ret,
                    tbsys::CNetUtil::addrToString(peer_id).c_str());
            message->reply_message(new StatusMessage(STATUS_MESSAGE_OK));
            return TFS_SUCCESS;
        }
        ...
        int DataService::crc_error_cmd(CrcErrorMessage* message)
        {
            ...
            TBSYS_LOG(
                    ...
                    check_file_item->block_id_, check_file_item->file_id_, check_file_item->crc_,
                    check_file_item->flag_,
        ...
        int32_t DataService::client_command(ClientCmdMessage* message)
        {
            ...
            do {
                ...
                if (CLIENT_CMD_SET_PARAM==type) {
                    ...
                    TBSYS_LOG(DEBUG, "set run param block_id: %u, server: %" PRI64_PREFIX "u, from: %s", block_id,
                            server_id,
        ...
        {
            int ret = TFS_SUCCESS;
            if ((ret = SysParam::instance().load_data_server(CONFIG.get_config_file_name(), server_index_))
                    !=TFS_SUCCESS) {
        ...
        int DataService::batch_write_info(WriteInfoBatchMessage* message)
        {
            ...
            TBSYS_LOG(DEBUG, "write block fileinfo start, blockid: %u, cluster flag: %d, meta size: %d, peer id: %s",
                    block_id,
                    message->get_cluster(), static_cast<int32_t>(raw_metas->size()),
                    tbsys::CNetUtil::addrToString(peer_id).c_str());
            ...
            if (NULL!=message) {
                if (RESP_HEART_MESSAGE==message->get_message_type()) {
                    ...
                    if (resp_hb_msg->get_status()==HEART_NEED_SEND_BLOCK_INFO) {
                        ...
                        data_management_.add_new_expire_block(resp_hb_msg->get_expire_blocks(), NULL,
                                resp_hb_msg->get_new_blocks());
                    }
