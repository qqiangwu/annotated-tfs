...
namespace tfs {
    namespace dataserver {
        ...
        bool MMapFile::remap_file()
        {
            ...
            if (size_==mmap_file_option_.max_mmap_size_) {
                TBSYS_LOG(INFO, "already mapped max size, now size: %d, max size: %d", size_,
            ...
            TBSYS_LOG(INFO, "mremap start. fd: %d, now size: %d, new size: %d, old data: %p", fd_, size_, new_size,
                    data_);
            void* new_map_data = mremap(data_, size_, new_size, MREMAP_MAYMOVE);
            if (MAP_FAILED==new_map_data) {
                TBSYS_LOG(ERROR, "ensure file size failed in mremap. new size: %d, error desc: %s", new_size,
                        strerror(errno));
                return false;
            }
            else {
                TBSYS_LOG(INFO, "mremap success. fd: %d, now size: %d, new size: %d, old data: %p, new data: %p", fd_,
                        size_, new_size,
                        data_, new_map_data);
            }
