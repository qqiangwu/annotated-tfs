 ...
 *
 *
 * Version: $Id: replicate_block.cpp 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
    ...
    namespace dataserver {
        ...
        int ReplicateBlock::run_replicate_block()
        {
            ...
            while (!stop_) {
                ...
                TBSYS_LOG(INFO, "replicate %s blockid: %u, %s=>%s, cost time: %d (ms)", (ret ? "fail" : "success"),
                        b->block_id_, tbsys::CNetUtil::addrToString(b->source_id_).c_str(),
        ...
        int ReplicateBlock::replicate_block_to_server(const ReplBlock* b)
        {
            ...
            if (TFS_ERROR==client->connect()) {
                ...
                TBSYS_LOG(ERROR, "connect to %s fail, blockid: %u", tbsys::CNetUtil::addrToString(ds_ip).c_str(),
                        block_id);
                return TFS_ERROR;
            }
