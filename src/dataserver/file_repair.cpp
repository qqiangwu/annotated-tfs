 ...
 *
 *
 * Version: $Id: file_repair.cpp 10 2010-10-11 01:57:47Z duanfei@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace dataserver {
        ...
        int FileRepair::fetch_file(const CrcCheckFile& crc_check_record, char* tmp_file)
        {
            ...
            for (; i<MAX_CONNECT_SERVERS; ++i) {
                ...
                if (tfs_client_->get_last_elect_ds_id()==dataserver_id_ || find(crc_check_record.fail_servers_.begin(),
                        crc_check_record.fail_servers_.end(), tfs_client_->get_last_elect_ds_id())
                        !=crc_check_record.fail_servers_.end()) {
            ...
            int fd = 0;
            get_tmp_file_name(tmp_file, SYSPARAM_DATASERVER.work_dir_.c_str(), crc_check_record.block_id_,
                    crc_check_record.file_id_);
            ...
            if (crc!=finfo.crc_ || crc!=crc_check_record.crc_ || total_size!=finfo.size_) {
                TBSYS_LOG(ERROR,
                        ...
                        fsname.get_name(), crc_check_record.block_id_, crc_check_record.file_id_, crc, finfo.crc_,
                        crc_check_record.crc_,
        ...
        int FileRepair::repair_file(const CrcCheckFile& crc_check_record, const char* tmp_file)
        {
            ...
            if (TFS_SUCCESS!=ret) {
                TBSYS_LOG(ERROR,
                        "repair file fail, ret: %d, errmsg: %s, tfsname: %s, blockid: %u fileid: %" PRI64_PREFIX "u\n",
                        ret, tfs_client_->get_file_name(), crc_check_record.block_id_, crc_check_record.file_id_,
                        tfs_client_->get_error_message());
            }
            else {
        ...
        void
        FileRepair::get_tmp_file_name(char* buffer, const char* path, const uint32_t block_id, const uint64_t file_id)
        {
