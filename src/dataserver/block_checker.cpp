...
namespace tfs {
    namespace dataserver {
        ...
        int BlockChecker::expire_error_block()
        {
            ...
            int64_t end_time = Func::curr_time();
            TBSYS_LOG(INFO, "end expire error block expire blocks: %d, cost time: %" PRI64_PREFIX "d",
                    expire_block_nums,
                    (end_time-start_time));
            return TFS_SUCCESS;
        }
        ...
        int BlockChecker::check_block(const LogicBlock* logic_block)
        {
            ...
            if (logic_block->get_crc_error()>=SYSPARAM_DATASERVER.max_crc_error_nums_ || logic_block->get_eio_error()
                    >=(int) SYSPARAM_DATASERVER.max_eio_error_nums_
                    || (((logic_block->get_crc_error()+logic_block->get_eio_error()
                            *(SYSPARAM_DATASERVER.max_crc_error_nums_/SYSPARAM_DATASERVER.max_eio_error_nums_))
                            >=SYSPARAM_DATASERVER.max_crc_error_nums_))) {
                TBSYS_LOG(ERROR,
                        "block may be error, will be removed. blockid: %u, crc error nums: %d, eio error nums: %d\n",
        ...
        int BlockChecker::set_error_bitmap(LogicBlock* logic_block)
        {
            ...
            if (physic_block_list) {
                phy_block_ids.clear();
                for (list<PhysicalBlock*>::iterator lit = physic_block_list->begin();
                     lit!=physic_block_list->end();
                     ++lit) {
