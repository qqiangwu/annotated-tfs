 ...
 *
 *
 * Version: $Id: blockfile_manager.cpp 48 2010-11-15 07:23:59Z zongdai@taobao.com $
 *
 * Authors:
    ...
    namespace dataserver {
                ...
                const BlockType block_type)
        {
            ...
            if (pmit!=physcial_blocks_.end()) {
                TBSYS_LOG(ERROR, "bitmap and physical blocks conflict. fatal error! physical block id: %u",
                        physical_block_id);
                assert(false);
            }
            ...
            do {
                ...
                ret = t_logic_block->init_block_file(super_block_.hash_slot_size_, super_block_.mmap_option_,
                        block_type);
                if (TFS_SUCCESS!=ret)
                    break;
                ...
                uint32_t& ext_physical_block_id, PhysicalBlock** physic_block)
        {
            ...
            if (pmit!=physcial_blocks_.end()) {
                TBSYS_LOG(ERROR, "physical block conflict. fatal error! ext physical blockid: %u",
        ...
        int BlockFileManager::expire_compact_blk(const time_t time, std::set<uint32_t>& erase_blocks)
        {
            ...
            for (LogicBlockMapIter mit = compact_logic_blocks_.begin(); mit!=compact_logic_blocks_.end();) {
                ...
                if ((mit->second) && (mit->second->get_last_update()<time)) {
                    TBSYS_LOG(INFO, "compact block is expired. blockid: %u, now: %ld, last update: %ld", mit->first,
                            time,
        ...
        int BlockFileManager::query_space(int64_t& used_bytes, int64_t& total_bytes) const
        {
            int64_t used_main_bytes = static_cast<int64_t> (super_block_.used_block_count_)
                    *static_cast<int64_t> (super_block_.main_block_size_)
                    +static_cast<int64_t> (super_block_.used_block_count_
        ...
        int BlockFileManager::load_block_file()
        {
            ...
            for (uint32_t pos = 1; pos<=static_cast<uint32_t> (super_block_.main_block_count_); ++pos) {
                ...
                if (normal_bit_map_->test(pos)) {
                    ...
                    if (TFS_SUCCESS!=ret) {
                        TBSYS_LOG(ERROR, "init physical block fail. fatal error! pos: %d, file: %s", pos,
                    ...
                    if (0==block_prefix.logic_blockid_) {
                        tbsys::gDelete(t_physical_block);
                        TBSYS_LOG(ERROR, "block prefix illegal! logic blockid: %u, physical pos: %u.",
                    ...
                    while (0!=block_prefix.next_physic_blockid_) {
                        ...
                        if (!normal_bit_map_->test(ext_pos)) {
                            TBSYS_LOG(ERROR,
                                    "ext next physicblock and bitmap conflicted! logic blockid: %u, physical blockid: %u.",
                        ...
                        if (prev_block_id!=block_prefix.prev_physic_blockid_) {
                            TBSYS_LOG(ERROR,
                                    "read prev blockid conflict! prev blockid: %u. block prefix's physic prev blockid: %u",
                            ...
                            // write or flush fail, just log, not exit. not dirty global ret value
                            int tmp_ret;
                            if ((tmp_ret = super_block_impl_->write_bit_map(normal_bit_map_, error_bit_map_))
                                    !=TFS_SUCCESS)
                                TBSYS_LOG(ERROR, "write bit map fail %d", tmp_ret);
                            if ((tmp_ret = super_block_impl_->flush_file())!=TFS_SUCCESS)
                                TBSYS_LOG(ERROR, "flush super block fail %d", tmp_ret);
                        ...
                        TBSYS_LOG(
                                ...
                                ext_pos, prev_block_id, block_prefix.prev_physic_blockid_,
                                block_prefix.next_physic_blockid_,
                                block_prefix.logic_blockid_);
                        pit = physcial_blocks_.find(ext_pos);
                        if (pit!=physcial_blocks_.end()) {
                            ret = EXIT_PHYSIC_UNEXPECT_FOUND_ERROR;
                            TBSYS_LOG(ERROR, "logic blockid: %u, physical blockid: %u is repetitive. fatal error!",
                                    logic_block_id,
                                    ext_pos);
                            break;
                        }
                    ...
                    if (TFS_SUCCESS!=ret && EXIT_COMPACT_BLOCK_ERROR!=ret && EXIT_BLOCKID_ZERO_ERROR!=ret
                        ...
                        break;
                            ...
                            !=ret) // ret == EXIT_COMPACT_BLOCK_ERROR || ret == EXIT_BLOCKID_CONFLICT_ERROR || EXIT_INDEX_CORRUPT_ERROR
                    {
                        // roll back
                        // can not make sure the type of this block, so add to confuse type
                        TBSYS_LOG(WARN, "logicblock status abnormal, need delete! logic blockid: %u. ret: %d",
                                logic_block_id, ret);
                        del_block(logic_block_id, C_CONFUSE_BLOCK);
                        ret = TFS_SUCCESS;
                    }
        ...
        {
            ext_block_count = static_cast<int32_t> (static_cast<float> (avail_data_space)
                    /(super_block_.block_type_ratio_
                            *static_cast<float> (super_block_.main_block_size_)
                            +static_cast<float> (super_block_.extend_block_size_)));
            main_block_count = static_cast<int32_t> (static_cast<float> (ext_block_count)
                    *super_block_.block_type_ratio_);
            TBSYS_LOG(INFO,
        ...
        int BlockFileManager::create_block(const BlockType block_type)
        {
            ...
            for (int32_t i = 1; i<=block_count; ++i) {
                ...
                if (C_MAIN_BLOCK==block_type) {
                    ...
                    tmp_stream << super_block_.mount_point_ << EXTENDBLOCK_DIR_PREFIX
                               << (i+super_block_.main_block_count_);
                }
                ...
                if (TFS_SUCCESS!=ret) {
                    TBSYS_LOG(ERROR, "write block file error. physcial block id: %d, block type: %d, ret: %d.", i,
                            block_type,
                            ret);
                    return ret;
                }
