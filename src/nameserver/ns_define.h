 ...
 *
 *
 * Version: $Id: ns_define.h 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
...
namespace tfs {
    ...
    namespace nameserver {
        struct NsGlobalInfo {
            NsGlobalInfo()
                    :
                    use_capacity_(0), total_capacity_(0), total_block_count_(0), total_load_(0), max_load_(1),
                    max_block_count_(1),
            ...
            NsGlobalInfo(uint64_t use_capacity, uint64_t totoal_capacity, uint64_t total_block_count,
                    int32_t total_load,
                    int32_t max_load, int32_t max_block_count, int32_t alive_server_count)
                    :
                    use_capacity_(use_capacity), total_capacity_(totoal_capacity),
                    total_block_count_(total_block_count),
                    total_load_(total_load), max_load_(max_load), max_block_count_(max_block_count),
                    alive_server_count_(
                            alive_server_count)
            {
...
,sync oplog flag(%s), last owner check time(%s), last push owner check packet time(%s)",
                        ...
                        destroy_flag_
                                ==NS_DESTROY_FLAGS_NO ? "no" : destroy_flag_==NS_DESTROY_FLAGS_YES ? "yes" : "unknow",
                        owner_role_
                                ==NS_ROLE_MASTER ? "master" : owner_role_==NS_ROLE_SLAVE ? "slave" : "unknow",
                        other_side_role_
                                ==NS_ROLE_MASTER ? "master" : other_side_role_==NS_ROLE_SLAVE ? "slave" : "unknow",
                        owner_status_
                                ==NS_STATUS_UNINITIALIZE ? "uninitialize"
                                                         : owner_status_==NS_STATUS_OTHERSIDEDEAD ? "other side dead" :
                                                           owner_status_
                        ...
                        other_side_status_
                                ==NS_STATUS_UNINITIALIZE ? "uninitialize"
                                                         : other_side_status_==NS_STATUS_OTHERSIDEDEAD
                                                           ? "other side dead" : other_side_status_
                                                                                         ==NS_STATUS_ACCEPT_DS_INFO
                                                                                 ? "accepct ds info"
                                                                                 : other_side_status_
                                                                                           ==NS_STATUS_INITIALIZED
                                                                                   ? "initialize" : "unknow",
                        sync_oplog_flag_
                                ==NS_SYNC_DATA_FLAG_NONE ? "none" : sync_oplog_flag_==NS_SYNC_DATA_FLAG_NO ? "no" :
                                                                    sync_oplog_flag_
                                                                            ==NS_SYNC_DATA_FLAG_READY ? "ready" :
                                                                    sync_oplog_flag_==NS_SYNC_DATA_FLAG_YES ? "yes"
                                                                                                            : "unknow",
                        last_owner_check_time_.toDateTime().c_str(),
                        last_push_owner_check_packet_time_.toDateTime().c_str());
            }
        };
    }
}
