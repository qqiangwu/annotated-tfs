        ...
        int HeartManagement::join_ds(Message* msg)
        {
            ...
            if (message->get_has_block()==HAS_BLOCK_FLAG_YES) {
                ...
                if ((meta_mgr_.get_fs_name_system()->get_ns_global_info()->switch_time_!=0) &&
                    ...
                    TBSYS_LOG(DEBUG, "new ds block count(%d): old ds block count(%d)", ds_stat_info->block_count_,
                            block_count);
                ...
                if (tmsg->getPCode()==MASTER_AND_SLAVE_HEART_RESPONSE_MESSAGE) {
                    if (tmsg->get_role()==NS_ROLE_SLAVE) {
                        if (ngi->other_side_status_!=tmsg->get_status()) // update otherside status
                        {
                            ...
                            if ((other_side_status==NS_STATUS_INITIALIZED)
            ...
            do {
                ...
                if ((iret==TFS_SUCCESS) && (rmsg!=NULL)) {
                    ...
                    if (tmsg->getPCode()==MASTER_AND_SLAVE_HEART_RESPONSE_MESSAGE) {
                        ...
                        TBSYS_LOG(WARN, "do master and slave heart fail: owner role(%s), other side role(%s)",
                                ngi->owner_role_
                                        ==NS_ROLE_MASTER ? "master" : "slave",
                                tmsg->get_role()==NS_ROLE_MASTER ? "master" : "slave");
                    }
                }
                tbsys::gDelete(rmsg);
            }
        ...
        int MasterAndSlaveHeartManager::do_master_msg(Message* message, void*)
        {
            ...
            if (mashm->get_role()!=NS_ROLE_SLAVE) {
                TBSYS_LOG(WARN, "do master and slave heart fail: owner role(%s), other side role(%s)", ngi->owner_role_
                                                                                                       ...
                                                                                                       ? "master"
                                                                                                       : "slave",
                        mashm->get_role()==NS_ROLE_MASTER ? "master" : "slave");
                return TFS_SUCCESS;
            }
