 ...
 *
 *
 * Version: $Id: replicate.cpp 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
    ...
    namespace nameserver {
        ...
        int ReplicateLauncher::build_plan(const VUINT32& lose_block_list)
        {
            ...
            for (uint32_t i = 0; i<lose_block_size; ++i) {
                ...
                TBSYS_LOG(DEBUG, "build_plan block(%u) replicate from (%s) to (%s)", block_id,
        ...
        int ReplicateLauncher::balance()
        {
            ...
            if (executor_.get_replicating_map().size()>=static_cast<uint32_t> (ds_size/2)) {
                TBSYS_LOG(ERROR, "replicating size(%u) > ds_size(%d)", executor_.get_replicating_map().size(),
                        ds_size/2);
                return TFS_SUCCESS;
            }
            ...
            for (iter = ds_maps->begin(); iter!=ds_maps->end(); ++iter) {
                ...
                current_average_block_size = (
                        (ds_stat_info->total_capacity_*SYSPARAM_NAMESERVER.max_use_capacity_ratio_)
                ...
                TBSYS_LOG(INFO,
                        "dataserver(%s), should block count(%"PRI64_PREFIX"d), current block count(%"PRI64_PREFIX"d)",
            ...
            for (; ds_src_iter!=ds_src_maps.end(); ++ds_src_iter) {
                ...
                for (; block_list_iter!=blks.end(); ++block_list_iter) {
                    ...
                    bool bret = elect_move_dest_ds(dest_ds_list_desc, ds_dest_counter, ds_list, ds_src_iter->first,
                            server_id);
                    if (!bret) {
            ...
            for (; rep_iter!=need_move_block_map.end(); ++rep_iter) {
                ...
                TBSYS_LOG(DEBUG, "move plan: block:%u, from %s to %s", replicate_block->block_id_,
                        tbsys::CNetUtil::addrToString(replicate_block->source_id_).c_str(),
                        tbsys::CNetUtil::addrToString(
            ...
            while (uncomplete_move_block_num && retry<10) {
                ...
                for (uint32_t i = 0; i<need_move_block_list_size; ++i) {
                    ...
                    if (replicate_block->block_id_==0)
                        continue;
                    if (executor_.is_replicating_block(replicate_block,
                            SYSPARAM_NAMESERVER.replicate_max_count_per_server_)) {
        ...
        int ReplicateExecutor::execute(const uint32_t block_id, void*)
        {
            ...
            if (current_ds_size>=SYSPARAM_NAMESERVER.min_replication_) {
                TBSYS_LOG(INFO, "block(%u), current_ds_size(%d) >= min_replication(%d), must be stop replicate",
                        block_id,
                ...
                const uint32_t block_id, int32_t is_move)
        {
            ...
            if (iret!=TFS_SUCCESS) {
                ...
                TBSYS_LOG(ERROR, "send replicate command faild, block(%u), %s===>%s, is_move(%s)", block_id,
                        tbsys::CNetUtil::addrToString(source_id).c_str(),
                        tbsys::CNetUtil::addrToString(destination_id).c_str(),
        ...
        int ReplicateExecutor::complete(const ReplBlock* cmd, bool success)
        {
            ...
            TBSYS_LOG(DEBUG, "Replicate Complete Result: block_id: %u,src_ds: %s, dest_ds: %s,"
                    ...
                    tbsys::CNetUtil::addrToString(destination_id).c_str(),
                    is_move==REPLICATE_BLOCK_MOVE_FLAG_YES ? "yes"
            ...
            if (success && block_collect!=NULL) {
                ...
                if (is_move && block_collect->get_ds()->size()
                        >static_cast<uint32_t> (SYSPARAM_NAMESERVER.min_replication_)) {
            ...
            if (delret>=0) {
                TBSYS_LOG(INFO, "%s %s, %s to %s block(%u) is_move(%s)", is_move ? "move" : "replicate",
                        rsucc ? "succeed"
        ...
        int ReplicateExecutor::execute(const vector<ReplBlock*> replicate_paln, void* args)
        {
            ...
            while (done && retry<10) {
                ...
                for (i = 0; i<replicate_paln_size; ++i) {
                    ...
                    send_replicate_cmd(replicate_block->source_id_, replicate_block->destination_id_,
                            replicate_block->block_id_,
                ...
                Func::sleep(SYSPARAM_NAMESERVER.replicate_check_interval_,
                        reinterpret_cast<int32_t*> (&laucher->destroy_flag_));
                if ((laucher->destroy_flag_==NS_DESTROY_FLAGS_YES) || (laucher->pause_flag_
                        & ReplicateLauncher::PAUSE_FLAG_PAUSE)) {
                    break;
                }
