 ...
 *
 *
 * Version: $Id: strategy.cpp 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace nameserver {
        ...
        void BaseStrategy::normalize(const ServerCollect* server_collect) const
        {
            ...
            primary_writable_block_count_ = percent(primary_writable_block_count,
        ...
        void dump_weigths(const multimap<int32_t, ServerCollect*>& weights)
        {
            ...
            while (it!=weights.end()) {
                printf("server(%s), weight(%d)", tbsys::CNetUtil::addrToString(it->second->get_ds()->id_).c_str(),
                        it->first);
                ++it;
            }
        ...
        {
            vector<ServerCollect*>::const_iterator maxit = std::max_element(ds_list.begin(), ds_list.end(),
                    CompareLoad());
        ...
        int32_t elect_replicate_dest_ds(const LayoutManager& meta,
                const ReplicateSourceStrategy::counter_type& dest_counter, const int32_t elect_count,
                VUINT64& elect_ds_list)
        {
        ...
        bool elect_move_dest_ds(const vector<ServerCollect*>& ds_list,
                const ReplicateDestStrategy::counter_type& dest_counter, const VUINT64& elect_ds_list,
                const uint64_t src_ds,
                uint64_t& dest_ds)
        {
            vector<ServerCollect*>::const_iterator maxit = std::max_element(ds_list.begin(), ds_list.end(),
                    CompareLoad());
