 ...
 *
 *
 * Version: $Id: compact.cpp 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace nameserver {
        ...
        bool CompactLauncher::is_compacting_block(const uint32_t block_id)
        {
            ...
            for (uint32_t i = 0; i<ds_status_list_size; ++i) {
                ...
                if ((status.second==COMPACT_COMPLETE_STATUS_START)
                        && check_compact_status(status.first, block_id, false,
         ...
         */
        int
        CompactLauncher::check_compact_status(const uint64_t server_id, const uint32_t block_id, const bool complete,
                const bool timeout)
        {
        ...
        int CompactLauncher::check_compact_ds(const vector <uint64_t>& ds_list)
        {
            ...
            for (uint32_t i = 0; i<ds_size; ++i) {
                ...
                if (server_collect==NULL) {
                    TBSYS_LOG(WARN, "server collect not found in dataserver map by (%s), not compact",
                            CNetUtil::addrToString(
                                    ds_list.at(i)).c_str());
                    return EXIT_DATASERVER_NOT_FOUND;
                }
                if (server_collect->get_ds()->current_load_>SYSPARAM_NAMESERVER.compact_max_load_) {
                    TBSYS_LOG(WARN, "current load > compact max load, not compact",
                            server_collect->get_ds()->current_load_,
                ...
                if (time_out_flag==COMPACT_STATUS_OK) {
                    TBSYS_LOG(WARN, "dataserver(%s) compact timeout, not compact",
                            CNetUtil::addrToString(ds_list.at(i)).c_str());
                    return EXIT_GENERAL_ERROR;
                }
            }
            return TFS_SUCCESS;
        ...
        int CompactLauncher::check_compact_complete(CheckBlockCompactParam& cbcp, VUINT64& ds_list, const BlockInfo*)
        {
            TBSYS_LOG(DEBUG, "check compacting complete in dataserver(%s) block(%u),status(%s)", CNetUtil::addrToString(
                    ...
                    cbcp.status_==COMPACT_COMPLETE_STATUS_SUCCESS ? "success" : "failed");
            ...
            if (check_compact_status(cbcp.id_, cbcp.block_id_, true, false)<COMPACT_STATUS_OK) {
                TBSYS_LOG(ERROR, " dataserver(%s) block(%u) not found in compact map, maybe expired",
                        CNetUtil::addrToString(
                                cbcp.id_).c_str(), cbcp.block_id_);
            }
            ...
            if (iter==compacting_block_map_.end()) {
                TBSYS_LOG(ERROR,
                        "dataserver(%s) block(%u) not found in compactiong block map, maybe timeout complete message",
            ...
            for (uint32_t i = 0; i<ds_status_list.size(); ++i) {
                ...
                if (status.first==cbcp.id_) {
                    ...
                    if (status.second==COMPACT_COMPLETE_STATUS_START) {
                        ...
                        if (time_out_flag==COMPACT_STATUS_OK) { // not timeout yet, wait
