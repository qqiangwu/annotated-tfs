 ...
 *
 *
 * Version: $Id: compact.h 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
    ...
    namespace nameserver {
        ...
        struct CheckBlockCompactParam {
            ...
            CheckBlockCompactParam(uint64_t dsid, uint32_t block_id, uint32_t st)
                    :
                    id_(dsid), block_id_(block_id), status_(st), all_success_(true), has_success_(false),
                    is_complete_(true),
         ...
         *
         */
        class CompactLauncher : public ProactorDataPipe<PipeDataAdaptor<uint32_t>, CompactLauncher

        > , public Launcher {
        public:
        typedef ProactorDataPipe <PipeDataAdaptor<uint32_t>, CompactLauncher> base_type;
        typedef hash_map<uint64_t, DsCompacting*, hash<uint64_t> >
        COMPACTING_SERVER_MAP;
        typedef hash_map <uint32_t, vector<pair<uint64_t, int32_t>> >
        COMPACTING_BLOCK_MAP;
