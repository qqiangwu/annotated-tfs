...
namespace tfs {
    namespace nameserver {
        ...
        void LeaseClerk::clear()
        {
            ...
            TBSYS_LOG(INFO, "cleanup lease complete, current lease map size(%u), consume(%" PRI64_PREFIX "u)",
                    lease_map_.size(), end-now);
        }
        ...
        uint32_t LeaseClerk::register_lease(const uint32_t block_id)
        {
            ...
            if (wait_count>SYSPARAM_NAMESERVER.max_wait_write_lease_) {
                TBSYS_LOG(WARN, "lease(%u), current wait thread(%"PRI64_PREFIX"d) beyond max_wait(%d)",
                        lease->lease_id_, wait_count,
            ...
            TBSYS_LOG(DEBUG, "register_lease block(%u) wait end ret(%d), valid(%d)", block_id, ret,
