...
namespace tfs {
    namespace nameserver {
                ...
                fs_name_system_(fsnm)
        {
            ...
            TBSYS_LOG(INFO, "ownerchecktime(%"PRI64_PREFIX"d)(us), maxownerchecktime(%"PRI64_PREFIX"u)(us)",
                    owner_check_time_.toMicroSeconds(),
        ...
        void OwnerCheckTimerTask::runTimerTask()
        {
            ...
            do {
                ...
                if ((ngi->last_push_owner_check_packet_time_>ngi->last_owner_check_time_+owner_check_time_) // >= 1
                        && (ngi->last_push_owner_check_packet_time_
                                <=ngi->last_owner_check_time_+max_owner_check_time_)) // <= 4
        ...
        NameServer::NameServer()
                :
                meta_mgr_(this), timer_(new tbutil::Timer()), master_slave_heart_mgr_(&meta_mgr_, timer_),
                main_task_queue_size_(
                        100), replicate_thread_(meta_mgr_), compact_thread_(meta_mgr_), redundant_thread_(meta_mgr_),
                heart_mgr_(
        ...
        int NameServer::start()
        {
            ...
            if ((ret!=TFS_SUCCESS)
                ...
                TBSYS_LOG(ERROR, "iret != TFS_SUCCESS or owner role(%s) == other side role(%s), must be exit...",
                        ns_global_info_.owner_role_==NS_ROLE_MASTER ? "master" : "slave",
                        ns_global_info_.other_side_role_
            ...
            else { //if we're the slave ns, we must sync data from the master ns.
            ...
            ret = timer_->scheduleRepeated(master_heart_task_,
                    tbutil::Time::seconds(SYSPARAM_NAMESERVER.heart_interval_));
            if (ret<0)
                return EXIT_GENERAL_ERROR;

            slave_heart_task_ = new SlaveHeartTimerTask(&meta_mgr_, timer_);
            ret = timer_->scheduleRepeated(slave_heart_task_,
                    tbutil::Time::seconds(SYSPARAM_NAMESERVER.heart_interval_));
            if (ret<0)
                return EXIT_GENERAL_ERROR;
...
///////////////////////////////////////////////////////////////////////////
        ...
        {
            Func::sleep(SYSPARAM_NAMESERVER.safe_mode_time_*10,
                    reinterpret_cast<int32_t*> (&ns_global_info_.destroy_flag_));
            while (ns_global_info_.destroy_flag_==NS_DESTROY_FLAGS_NO) {
                if (ns_global_info_.owner_role_==NS_ROLE_MASTER)
        ...
        int NameServer::do_check_blocks()
        {
            ...
            while (ns_global_info_.destroy_flag_==NS_DESTROY_FLAGS_NO) {
                ...
                ScannerManager::Scanner compact_scanner(need_check[2] && compact_thread_.is_compacting_time(),
                        server_size,
                        compact_thread_, compact_blocks);
                ScannerManager::Scanner redundant_scanner(need_check[3], server_size, redundant_thread_,
                        redundant_blocks);
                ...
                TBSYS_LOG(DEBUG, "build plan : lose (%u), compact (%u), redundant (%u)", lose_blocks.size(),
                        compact_blocks.size(), redundant_blocks.size());
        ...
        {
            Func::sleep(SYSPARAM_NAMESERVER.safe_mode_time_*20,
                    reinterpret_cast<int32_t*> (&ns_global_info_.destroy_flag_));

            while (ns_global_info_.destroy_flag_==NS_DESTROY_FLAGS_NO) {
                if (ns_global_info_.owner_role_==NS_ROLE_MASTER) {
                    // wait for ds report
                    if ((ns_global_info_.switch_time_!=0)
                            && (time(NULL)<static_cast<time_t> (ns_global_info_.switch_time_
                                    +SYSPARAM_NAMESERVER.safe_mode_time_))) {
                        float wait_for_ds_report_time = static_cast<float> ((ns_global_info_.switch_time_
                                +SYSPARAM_NAMESERVER.safe_mode_time_)-time(NULL));
                        TBSYS_LOG(INFO, "not balance, wait(%f) for ds report...", wait_for_ds_report_time);
                        Func::sleep(wait_for_ds_report_time,
                                reinterpret_cast<int32_t*> (&ns_global_info_.destroy_flag_));
                    }
...
//////////////////////////////////////////////////////////////////////////////
        tbnet::IPacketHandler::HPRetCode NameServer::handlePacket(tbnet::Connection* connection, tbnet::Packet* packet)
        {
            ...
            if (ns_global_info_.owner_status_==NS_STATUS_UNINITIALIZE) {
                ...
                if (ns_global_info_.owner_role_==NS_ROLE_MASTER) {
                    ...
                    if (pcode==MASTER_AND_SLAVE_HEART_MESSAGE || pcode==SET_DATASERVER_MESSAGE
        ...
        int NameServer::get_block_info(Message* msg)
        {
            ...
            if (mode & BLOCK_READ) {
                ...
                TBSYS_LOG(DEBUG,
                        "get block info: block(%u) mode(%d) lease(%u), version(%d), dataserver size(%u), result(%d)",
            ...
            if (ret!=TFS_SUCCESS) {
                ...
                return MessageFactory::send_error_message(message, TBSYS_LOG_LEVEL(ERROR), ret,
                        ns_global_info_.owner_ip_port_,
        ...
        int NameServer::write_complete(Message* msg)
        {
            ...
            TBSYS_LOG(DEBUG, "write commit: block(%u) dataserver(%s) version(%d), file_count(%d)"
                            "size(%d) delfile_count(%d), del_size(%d), seq_no(%u), lease(%u), unlink(%s), status(%s)",
                    blk->block_id_,
        ...
        int NameServer::update_block_info(Message* msg)
        {
            ...
            if (repair_flag==UPDATE_BLOCK_NORMAL) {
                ...
                if (ret==TFS_SUCCESS) {
                    ...
                    message->reply_message(
                            new StatusMessage(ret, "new block info version lower than meta, cannot update"));
                }
                return ret;
            }
            ...
            if (block_collect==NULL) {
                ptr->mutex_.unlock();
                MessageFactory::send_error_message(message, TBSYS_LOG_LEVEL(ERROR), EXIT_BLOCK_NOT_FOUND,
                        ns_global_info_.owner_ip_port_, "repair block, block collect not found by block id(%u)",
                        block_id);
                return TFS_SUCCESS;
            }
            ...
            if (source_ds_id==0) {
                MessageFactory::send_error_message(message, TBSYS_LOG_LEVEL(ERROR), EXIT_NO_DATASERVER,
                        ns_global_info_.owner_ip_port_,
                        "repair block(%u) no any other dataserver(%u) hold a correct replica", block_id,
                        ds_list.size());
                return TFS_SUCCESS;
            }
            ...
            return replicate_thread_.get_executor().send_replicate_cmd(source_ds_id, dest_ds_id, block_id,
        ...
        int NameServer::owner_check(Message*)
        {
            ...
            TBSYS_LOG(INFO, "owner check time(%" PRI64_PREFIX "u: %s)",
                    ns_global_info_.last_owner_check_time_.toMicroSeconds(),
        ...
        {
            TBSYS_LOG(INFO, "remove  block (%u) on server (%s)", block_id,
        ...
        int NameServer::do_control_cmd(Message* msg)
        {
            ...
            do {
                if (type==CLIENT_CMD_LOADBLK) {
                    ...
                    TBSYS_LOG(WARN, "client(%s) exectue CLIENT_CMD_LOADBLK server(%s), block_id(%u)",
                    ...
                    resp->set_message(STATUS_MESSAGE_OK);
                    TBSYS_LOG(WARN, "client(%s) exectue CLIENT_CMD_EXPBLK server(%s), block_id(%u)",
                            tbsys::CNetUtil::addrToString(
                                    from_ds_id).c_str(), tbsys::CNetUtil::addrToString(ds_id).c_str(), block_id);
                }
                    ...
                    TBSYS_LOG(INFO, "immediately replicate block(%u) replicate block move flag(%s) src(%s) dest(%s)",
                            block_id,
                            replicate_move_flag==REPLICATE_BLOCK_MOVE_FLAG_NO ? "no" : "yes",
                            tbsys::CNetUtil::addrToString(
                    ...
                    if ((replicate_move_flag==REPLICATE_BLOCK_MOVE_FLAG_NO)
                        ...
                        if (src_ds_id==0 || dest_ds_id==0 || src_ds_id==dest_ds_id) {
                            TBSYS_LOG(ERROR,
                                    "immediately replicate block id(%u) replicate_move_flag(%s) src(%s) or dest(%s) is illegal",
                                    block_id, replicate_move_flag==REPLICATE_BLOCK_MOVE_FLAG_NO ? "no" : "yes",
                                    tbsys::CNetUtil::addrToString(src_ds_id).c_str(),
                                    tbsys::CNetUtil::addrToString(dest_ds_id).c_str());
                            break;
                        }
                        replicate_thread_.get_executor().send_replicate_cmd(src_ds_id, dest_ds_id, block_id,
                                replicate_move_flag);
                    }
                    else {
                    ...
                    TBSYS_LOG(INFO, "client(%s) exectue CLIENT_CMD_CLEAR_REPL_INFO",
                            tbsys::CNetUtil::addrToString(from_ds_id).c_str());
        ...
        int NameServer::get_peer_role()
        {
            ...
            while (true) {
                TBSYS_LOG(DEBUG, "get peers(%s) role, owner role(%s), other side role(%s)...",
                        tbsys::CNetUtil::addrToString(
                                ns_global_info_.other_side_ip_port_).c_str(),
                        ns_global_info_.owner_role_==NS_ROLE_MASTER ? "master"
                                                                    : "slave",
                        ns_global_info_.other_side_role_==NS_ROLE_MASTER ? "master" : "slave");
        ...
        void NameServer::initialize_handle_task_and_heart_threads()
        {
            ...
            TBSYS_LOG(INFO, "fsnamesystem::start: %s",
                    tbsys::CNetUtil::addrToString(ns_global_info_.owner_ip_port_).c_str());
        }
