 ...
 *
 *
 * Version: $Id: meta_manager.cpp 50 2010-11-17 02:04:35Z nayan@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace nameserver {
         ...
         */
        int
        MetaManager::report_blocks(const uint64_t ds_id, const vector <BlockInfo>& blocks, EXPIRE_BLOCK_LIST& expires)
        {
            ...
            if ((server_collect==NULL) || (!server_collect->is_alive())) {
                ...
                TBSYS_LOG(ERROR, "dataserver(%s)report,can't find server collect",
            ...
            for (uint32_t i = 0; i<blocks_size; ++i) {
                ...
                BlockChunkPtr ptr = meta_mgr_.get_block_chunk(new_block_info.block_id_);
                {
                    ...
                    if (__gnu_cxx::abs(block_info->version_-new_block_info.version_)>2) {
                        if (block_info->version_>new_block_info.version_) {
                            if (current_block_ds_size>0) {
                                ...
                                TBSYS_LOG(WARN,
                                        "block(%u) in dataserver(%s) version error(%d:%d), but not found dataserver",
                                        new_block_info.block_id_, tbsys::CNetUtil::addrToString(ds_id).c_str(),
                                        block_info->version_, new_block_info.block_id_);
                                memcpy(const_cast<BlockInfo*> (block_info), const_cast<BlockInfo*> (&new_block_info),
                                        BLOCKINFO_SIZE);
                            }
                        }
                            ...
                            if (!first) {
                                TBSYS_LOG(WARN,
                                        "block(%u) in dataserver(%s) version error(%d:%d),replace ns version, current dataserver size(%u)",
                                        new_block_info.block_id_, tbsys::CNetUtil::addrToString(ds_id).c_str(),
                                ...
                                if (ngi->owner_role_==NS_ROLE_MASTER) {
                                    ...
                                    for (uint32_t k = 0; k<expire_ds_list.size(); ++k) {
                                        ...
                                        TBSYS_LOG(WARN, "release relation dataserver(%s), block(%u), result(%d)",
                                                tbsys::CNetUtil::addrToString(other_server_id).c_str(),
                                                block_info->block_id_, ret);
                                    }
                                }
                            }
                        }
                    ...
                    TBSYS_LOG(DEBUG, "force_be_master(%s), is_full(%s), ds_list_size(%u), mode=%d",
                            force_be_master ? "true" : "false",
                            block_collect->is_full() ? "true" : "fase", ds_list_size,
                            new_block_info.block_id_%ds_list_size);
                }
                ...
                TBSYS_LOG(DEBUG, "dataserver(%s) report: block(%u),version(%d),filecount(%d),"
                        ...
                        server_collect->get_primary_writable_block_list()->size(),
                        meta_mgr_.get_writable_block_list().size());
            }
            meta_mgr_.sort();
            return TFS_SUCCESS;
        }
            ...
            if ((mode & BLOCK_NEWBLK)) {
                if (block_collect==NULL) {
                    ...
                    if (block_collect->get_creating_flag()) {
                        ptr->mutex_.unlock();
                        TBSYS_LOG(ERROR, "block(%u) found meta data, but creating by another thread, must be return",
                                block_id);
                        return EXIT_NO_BLOCK;
                    }
                    else {
        ...
        int MetaManager::update_block_info(const BlockInfo& new_block_info, const uint64_t ds_id, bool addnew)
        {
            ...
            if (block_collect->get_block_info()->version_<new_block_info.version_) {
                memcpy(const_cast<BlockInfo*> (block_collect->get_block_info()),
                        const_cast<BlockInfo*> (&new_block_info),
                        BLOCKINFO_SIZE);
                ret = TFS_SUCCESS;
            }
            else {
            ...
            if (TFS_SUCCESS==ret) {
                ...
                if (oplog_sync_mgr_.log(&new_block_info, isnew ? OPLOG_INSERT : OPLOG_UPDATE, dsList)!=TFS_SUCCESS) {
                    TBSYS_LOG(ERROR, "log failed, block(%u), isnew(%s)", new_block_info.block_id_,
                            isnew==OPLOG_INSERT ? "insert" : isnew==OPLOG_UPDATE ? "update" : "unknow");
                }
            }
        ...
        BlockCollect* MetaManager::add_new_block(uint32_t& block_id, const uint64_t ds_id)
        {
            ...
            if (ds_id!=0) {
                ...
                if ((server_collect!=NULL)
                    ...
                    TBSYS_LOG(DEBUG, "add_new_block : server %s not writable",
                            tbsys::CNetUtil::addrToString(ds_id).c_str());
                    return NULL;
                }
            }
        ...
        int MetaManager::check_primary_writable_block(const uint64_t ds_id, const int32_t add_block_count, bool promote)
        {
            ...
            if (server_collect!=NULL) {
                ...
                if (current>=SYSPARAM_NAMESERVER.max_write_file_count_) {
                    TBSYS_LOG(INFO,
                            "check primary writableblock in dataserver(%s), current_primary_block_count(%u) >= max_write_file_count(%d), no need to add new block",
                            tbsys::CNetUtil::addrToString(ds_id).c_str(), current,
                ...
                TBSYS_LOG(INFO,
                        "check primary writableblock in dataserver(%s), current primary block count(%u), need add block count(%d)",
        ...
        int MetaManager::promote_primary_write_block(const ServerCollect* server_collect, int32_t& need_count)
        {
            ...
            if (writable_blocks.size()<=primary_writable_blocks.size()) {
                TBSYS_LOG(WARN,
                        "not found primary writable block, writable_block_size(%u) <= primary_writable_block_size(%u)",
            ...
            while (iter!=writable_blocks.end() && need_count>0) {
                ...
                if (primary_writable_blocks.find(*iter)==primary_writable_blocks.end()) {
                    ...
                    if ((block_collect!=NULL) && (!block_collect->is_full()) && (block_collect->get_master_ds()==0)
                            && (static_cast<int32_t> (block_collect->get_ds()->size())
