 ...
 *
 *
 * Version: $Id: oplog.cpp 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace nameserver {
        OpLog::OpLog(const std::string& logname, int maxLogSlotsSize)
                :
                MAX_LOG_SLOTS_SIZE(maxLogSlotsSize), MAX_LOG_BUFFER_SIZE(maxLogSlotsSize*MAX_LOG_SIZE), path_(logname),
                seqno_(
                        0), last_flush_time_(0), slots_offset_(0), fd_(-1),
                buffer_(new char[maxLogSlotsSize*MAX_LOG_SIZE+1])
        {
