...
namespace tfs {
    namespace nameserver {
        ...
        struct WriteLease {
                    ...
                    const int64_t max_wait_count) const
            {
                TBSYS_LOG(DEBUG, "block id(%u) last wirte time(%"PRI64_PREFIX"d), expire time(%"PRI64_PREFIX"d),"
                        ...
                        block_id, last_write_time_, expire_time_, tbsys::CTimeUtil::getTime(), lease_id_,
                        is_valid ? "true" : "false", current_wait_count, max_wait_count,
                        status_==WRITING ? "writing" : status_==DONE ? "done" : status_==FAILED ? "failed" :
                                                                                status_==EXPIRED ? "expired" : status_
                                                                                                                       ==CANCELED
                                                                                                               ? "canceled"
                                                                                                               : status_
                                                                                                                         ==OBSOLETE
        ...
        class LeaseClerk {
            typedef __gnu_cxx::hash_map<uint32_t, WriteLease*, hash<uint32_t> >
            LEASE_MAP;
        ...
        public:
            ...
            bool
            write_commit(common::BlockInfo* block_info, common::BlockInfo* new_block_info, const uint32_t lease_id);
