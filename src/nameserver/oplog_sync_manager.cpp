 ...
 *
 *
 * Version: $Id: oplog_sync_manager.cpp 18 2010-10-12 09:45:55Z nayan@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace nameserver {
        ...
        OpLogSyncManager::OpLogSyncManager(MetaManager* mm)
                :
                is_destroy_(false), meta_mgr_(mm), ns_fs_image_(mm), oplog_(NULL), file_queue_(NULL),
                file_queue_thread_(NULL)
        {
        ...
        int OpLogSyncManager::initialize(LayoutManager& block_ds_map)
        {
            ...
            snprintf(default_work_dir, MAX_PATH_LENGTH, "%s/nameserver",
                    CONFIG.get_string_value(CONFIG_PUBLIC, CONF_WORK_DIR));
        ...
        int OpLogSyncManager::register_msg(const Message* msg)
        {
            ...
            if ((ngi->owner_role_!=NS_ROLE_MASTER)
                    || (ngi->other_side_status_<NS_STATUS_ACCEPT_DS_INFO)//slave dead or uninitialize
        ...
        int OpLogSyncManager::rotate()
        {
            ...
            if (iret==TFS_SUCCESS) {
                ...
                TBSYS_LOG(
                        ...
                        head->read_seqno_, head->read_offset_, head->write_seqno_, head->write_filesize_,
                        head->queue_size_,
            ...
            do {
                ...
                TBSYS_LOG(DEBUG,
                        "time(%"PRI64_PREFIX"d), seqno(%d), length(%d), cmd(%s), BlockInfo: id(%u), version(%d), file_count(%d), size(%d),"
                                "delete_file_count(%d), delete_size(%d), seqno(%d), ds(%s)", header->time_,
                        header->seqno_, header->length_,
