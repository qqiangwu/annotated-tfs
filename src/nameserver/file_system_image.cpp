 ...
 *
 *
 * Version: $Id: file_system_image.cpp 18 2010-10-12 09:45:55Z nayan@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace nameserver {
        ...
        int
        FileSystemImage::initialize(LayoutManager& block_ds_map, const OpLogRotateHeader& head, FileQueue* file_queue)
        {
            ...
            TBSYS_LOG(DEBUG, "befor load queue header: read seqno(%d), read offset(%d), write seqno(%d),"
                            ...
                            "rotate offset(%d)", qhead->read_seqno_, qhead->read_offset_, qhead->write_seqno_,
                    qhead->write_filesize_,
            ...
            if (access(image_file_path_, R_OK)==0) {
                ...
                if (header_.total_bytes_!=total_byte || header_.total_file_count_!=total_file_count) {
                    TBSYS_LOG(ERROR,
                            "total_byte(%"PRI64_PREFIX"d) , (%"PRI64_PREFIX"d), total_file_count(%"PRI64_PREFIX"d), (%"PRI64_PREFIX"d)",
                            header_.total_bytes_, total_byte,
