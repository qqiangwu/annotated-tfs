...
TfsFile::TfsFile()
        :
        session_(NULL), client_(NULL), file_number_(0), file_id_(0), last_elect_ds_id_(0), block_id_(0), crc_(0),
        mode_(0),
...
/* set name of the file after a conversion */
...
/* open a file:
 *
 *      send requests to the nameserver
 *
 *  \pre    the file is associated with a session
 *  \post   retains a connection to the primary dataserver if all operations
 *          are successful
 */
int TfsFile::tfs_open(const uint32_t block_id, const uint64_t file_id, const int32_t mode)
{
    ...
    if (mode==READ_MODE) {
        // send a request to the nameserver and get all ds containing this block
        ...
        // ditto!
        // I don't know what UNLIKE_MODE means in the moment
    ...
    if ((mode_ & READ_MODE)) {
        ...
        // when write, just use the first one(primary specified by the nameserver)
        pri_ds_index_ = 0;
    }
...
/* release existing connection and reconnect.
 *
 * \pre     one client is already connected.
 * \post    one client is already connected.
 */
 ...
 *
 * \pre     one client is already connected.
 * \post    one client is already connected and a new file is created
 */
    ...
    while (retry--) {
        ...
        if (message!=NULL) {
            if (message->get_message_type()==RESP_READ_DATA_MESSAGE_V2) {
                ...
                if (offset_==0 && msg->get_file_info()==NULL) {   // read failed, retry
                    TBSYS_LOG(WARN,
                            "tfs read, get file info error, filename(%s), fileid(%"PRI64_PREFIX"u), form dataserver(%s)",
                    ...
                    if (file_info->id_!=file_id_) {
                        TBSYS_LOG(WARN,
                                "tfs read, file(%s  : %"PRI64_PREFIX"u  :%"PRI64_PREFIX"u) not match, from dataserver(%s)",
                                file_name_, file_info->id_, file_id_,
...
/* simply ignore this since it's not a core functionality */
        ...
        if (message!=NULL) {
            if (message->get_message_type()==RESP_READ_DATA_MESSAGE_V2) {
                ...
                if (offset_==0 && msg->get_file_info()==NULL) {
                    TBSYS_LOG(WARN,
                            "tfs_read_scale_image, get file info error, filename(%s), fileid(%"PRI64_PREFIX"u) from dataserer(%s)",
                            file_name_, file_id_, tbsys::CNetUtil::addrToString(client_->get_mip()).c_str());
                    tbsys::gDelete(message);
                    continue;
                }
                    ...
                    if (finfo->id_!=file_id_) {
                        TBSYS_LOG(WARN,
                                "tfs_read_scale_image, fileid(%s  %"PRI64_PREFIX"u :%"PRI64_PREFIX"u) not match, from dataserver(%s)",
                                file_name_, finfo->id_, file_id_,
                                tbsys::CNetUtil::addrToString(client_->get_mip()).c_str());
                        tbsys::gDelete(message);
                        continue;
                    }
...
#endif
                    snprintf(error_message_, ERR_MSG_SIZE,
                            "tfs_read_scale_image, read fail from dataserver(%s), ret len(%d)",
                            tbsys::CNetUtil::addrToString(client_->get_mip()).c_str(), ret_len);
                }
            }
            else if (message->get_message_type()==STATUS_MESSAGE) {
...
/*
 * \pre     connection is established and a file is opened
 */
...
/* we don't care about this since we've already known a better one */
...
/*
 * \pre a file is opened and in READ_MODE 
 */
int TfsFile::tfs_stat(FileInfo* file_info, const int32_t mode)
{
    ...
    while (retry--) {
        ...
        if (msg->get_file_info()!=NULL) {
            memcpy(file_info, msg->get_file_info(), FILEINFO_SIZE);
            if (file_info->id_!=file_id_) {
                TBSYS_LOG(WARN, "tfs stat fileid(%s : %"PRI64_PREFIX"u :%"PRI64_PREFIX"u not match", file_name_,
                        file_info->id_, file_id_);
                tbsys::gDelete(message);
                continue;
            }
            ret = TFS_SUCCESS;
...
/*
 * \pre     the file is opened and in WRITE_MODE 
 * \post    write successful or disconnect.
 *
 * Write once! No retry!
 *
 * Assumption: there are enough space inside the block.
 */
...
/*
 * If in READ_MODE, simply close.
 * If in WRITE_MODE, tell the nameserver.
 */
int TfsFile::tfs_close()
{
    ...
    if (message!=NULL) {
        ...
        snprintf(error_message_, ERR_MSG_SIZE,
                "tfs file close,send msg to dataserver fail, errors(time out, blockid(%u) fileid(%" PRI64_PREFIX "u)",
                block_id_, file_id_);
    }
...
// create a new file in an arbitry block.
