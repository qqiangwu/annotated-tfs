 ...
 *
 *
 * Version: $Id: ds_lib.h 5 2010-09-29 07:44:56Z duanfei@taobao.com $
 *
 * Authors:
 *   jihe
 *      - initial release
...
private:
    int send_message_to_ds(const uint64_t server_id, const tfs::message::Message* ds_msg, string& err_msg,
            tfs::message::Message** ret_msg);
    ...
    int
    create_file_num(const uint64_t server_id, const uint32_t block_id, const uint64_t file_id, uint64_t& new_file_id,
            int64_t& file_num);
