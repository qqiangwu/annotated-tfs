 ...
 *
 *
 * Version: $Id: ds_lib.cpp 21 2010-10-19 02:12:38Z nayan@taobao.com $
 *
 * Authors:
 *   jihe
 *      - initial release
    ...
    if ((ret_status==TFS_SUCCESS) && (ret_msg!=NULL)) {
        if (ret_msg->get_message_type()==BLOCK_FILE_INFO_MESSAGE) {
            ...
            for (i = 0; i<list_size; i++) {
                ...
                printf("%s %20" PRI64_PREFIX "u %10u %10u %10u %s %s %02d %10u\n", fsname.get_name(), file_info->id_,
                        file_info->offset_,
                        file_info->size_, file_info->usize_, Func::time_to_str(file_info->modify_time_).c_str(),
                        Func::time_to_str(
