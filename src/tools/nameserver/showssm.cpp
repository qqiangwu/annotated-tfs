    ...
    for (; it!=block_map->end(); it++) {
        ...
        if (!report || block_info->size_<max_block_size || block_info->size_>max_block_size*2) {
            fprintf(stdout, "%-10u %10d %10d %10d %10d %10d %5u %5d\n", block_info->block_id_, block_info->version_,
                    block_info->file_count_, block_info->size_,
                    block_info->del_file_count_, block_info->del_size_, block_info->seq_no_,
                    static_cast<int32_t>(server_list->size()));
        }
...
void print_server_map(SERVER_MAP* ds_map)
{
    ...
    for (i = 0; i<list_size; i++) {
        ...
        fprintf(stdout,
                "%-21s %7s %7s %2d%% %6d %4d %2d %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d %-19s\n",
    ...
    fprintf(stdout,
            "TOTAL %5Zd %9s %7s %7s %2d%% %6s %4d %2s %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d\n\n",
            ds_map->size(),
            "",
...
void print_machine(SERVER_MAP* ds_map, int32_t flag)
{
    ...
    for (int32_t i = 0; i<static_cast<int32_t> (ds_list.size()); i++) {
        ...
        if (0==strncmp(tmp_ip, tmp_ip_next, strlen(tmp_ip_next))) {
            ...
            if (flag==1) {
                fprintf(stdout,
                        "%-15s %-2d %6s %7s  %2d%%  %"PRI64_PREFIX"u   %u %6s %5"PRI64_PREFIX"d %6s %"PRI64_PREFIX"d %3s %"PRI64_PREFIX"d %5s %"PRI64_PREFIX"d %3s %"PRI64_PREFIX"d %5s %5"PRI64_PREFIX"d\n",
                        tmp_ip,
                        index_ip,
                ...
                fprintf(stdout,
                        "%-15s %-2d %6s %7s  %2d%%  %"PRI64_PREFIX"u   %u %3s %"PRI64_PREFIX"d %5s %5"PRI64_PREFIX"d %3s %5"PRI64_PREFIX"d %5s %5"PRI64_PREFIX"d %-19s\n",
                        tmp_ip,
                        index_ip,
                        Func::format_size(use_machine_cap).c_str(),
                        Func::format_size(total_machine_cap).c_str(),
        ...
        if (i==static_cast<int32_t>(ds_list.size())-1) {
            ...
            if (flag==1) {
                fprintf(stdout,
                        "%-15s %-2d %6s %7s  %2d%%  %"PRI64_PREFIX"u   %u %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d %3s %5"PRI64_PREFIX"d %5s %5"PRI64_PREFIX"d %3s %5"PRI64_PREFIX"d %5s %5"PRI64_PREFIX"d\n",
                        tmp_ip,
                        index_ip,
                        Func::format_size(use_machine_cap).c_str(),
                        Func::format_size(total_machine_cap).c_str(),
                ...
                fprintf(stdout,
                        "%-15s %-2d %6s %7s  %2d%%  %"PRI64_PREFIX"u   %u %3s %5"PRI64_PREFIX"d %5s %5"PRI64_PREFIX"d %3s %5"PRI64_PREFIX"d %5s %5"PRI64_PREFIX"d %-19s\n",
                        tmp_ip,
                        index_ip,
                        Func::format_size(use_machine_cap).c_str(),
                        Func::format_size(total_machine_cap).c_str(),
    ...
    if (1==flag) {
        fprintf(stdout,
                "Total : %-5d %-2Zd %6s %7s  %2d%%  %"PRI64_PREFIX"u   %u %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d\n",
                total_machine,
                ds_list.size(),
        ...
        fprintf(stdout,
                "Total : %-5d %-2Zd %6s %7s  %2d%%  %"PRI64_PREFIX"u   %u %6s %5"PRI64_PREFIX"d %6s %5"PRI64_PREFIX"d\n",
                total_machine,
                ds_list.size(),
                Func::format_size(use_cluster_cap).c_str(),
                Func::format_size(total_cluster_cap).c_str(),
