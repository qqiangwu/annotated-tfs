 ...
 *
 *
 * Version: $Id: tfstool.cpp 36 2010-11-02 08:14:59Z nayan@taobao.com $
 *
 * Authors:
...
int cmd_get_repl_info(TfsClient* tfs_client, VEC_STRING&)
{
    ...
    if (TFS_SUCCESS==send_message_to_server(nsip_port, &rep_i_msg, &ret_msg)) {
        if (ret_msg->get_message_type()==REPLICATE_INFO_MESSAGE) {
            ...
            for (ReplicateInfoMessage::COUNTER_TYPE::const_iterator it = src_counter.begin();
                 it!=src_counter.end();
                 ++it) {
            ...
            for (ReplicateInfoMessage::COUNTER_TYPE::const_iterator it = dst_counter.begin();
                 it!=dst_counter.end();
                 ++it) {
