 ...
 *
 *
 * Version: $Id: parameter.cpp 50 2010-11-17 02:04:35Z nayan@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace common {
        ...
        int SysParam::load(const string& tfsfile)
        {
            ...
            nameserver_.replicate_check_interval_ = CONFIG.get_int_value(CONFIG_NAMESERVER, CONF_REPL_CHECK_INTERVAL,
                    15);
            nameserver_.redundant_check_interval_
                    = CONFIG.get_int_value(CONFIG_NAMESERVER, CONF_REDUNDANT_CHECK_INTERVAL, 30);
            nameserver_.balance_check_interval_ = CONFIG.get_int_value(CONFIG_NAMESERVER, CONF_BALANCE_CHECK_INTERVAL,
                    300);
            ...
            nameserver_.replicate_max_count_per_server_ = CONFIG.get_int_value(CONFIG_NAMESERVER, CONF_REPL_MAX_COUNT,
                    3);
            nameserver_.compact_check_interval_ = CONFIG.get_int_value(CONFIG_NAMESERVER, CONF_COMPACT_CHECK_INTERVAL,
                    600);
            ...
            nameserver_.compact_preserve_time_ = CONFIG.get_int_value(CONFIG_NAMESERVER, CONF_COMPACT_PRESERVE_TIME,
                    300);
            ...
            nameserver_.add_primary_block_count_ = CONFIG.get_int_value(CONFIG_NAMESERVER, CONF_ADD_PRIMARY_BLOCK_COUNT,
                    3);
        ...
        int SysParam::load_data_server(const string& tfsfile, const string& index)
        {
            ...
            dataserver_.work_dir_ = change_index(
                    CONFIG.get_string_value(CONFIG_DATASERVER, CONF_WORK_DIR, default_work_dir),
                    string(""), server_index);
            dataserver_.log_file_ = change_index(
                    CONFIG.get_string_value(CONFIG_DATASERVER, CONF_LOG_FILE, default_log_file),
                    string(".log"), server_index);
            dataserver_.pid_file_ = change_index(
                    CONFIG.get_string_value(CONFIG_DATASERVER, CONF_LOCK_FILE, default_pid_file),
            ...
            dataserver_.expire_compact_time_ = CONFIG.get_int_value(CONFIG_DATASERVER, CONF_EXPIRE_COMPACTBLOCK_TIME,
                    86400);
            dataserver_.replicate_thread_count_ = CONFIG.get_int_value(CONFIG_DATASERVER, CONF_REPLICATE_THREADCOUNT,
                    3);
            //default use O_SYNC
        ...
        {
            if (CONFIG.get_string_value(CONFIG_DATASERVER, CONF_MOUNT_POINT_NAME)==NULL
                    || strlen(CONFIG.get_string_value(
                            CONFIG_DATASERVER, CONF_MOUNT_POINT_NAME))>=static_cast<uint32_t> (MAX_DEV_NAME_LEN)) {
                std::cerr << "mount name is invalid" << std::endl;
                return TFS_ERROR;
            }
            filesystem_param_.mount_name_ = string(
                    CONFIG.get_string_value(CONFIG_DATASERVER, CONF_MOUNT_POINT_NAME)).append(
                    index);
            ...
            filesystem_param_.super_block_reserve_offset_ = CONFIG.get_int_value(CONFIG_DATASERVER,
                    CONF_SUPERBLOCK_START, 0);
