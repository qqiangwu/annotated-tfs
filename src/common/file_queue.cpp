 ...
 *
 *
 * Version: $Id: file_queue.cpp 42 2010-11-12 07:32:15Z nayan@taobao.com $
 *
 * Authors:
...
QueueItem* FileQueue::pop(int index)
{
    ...
    if (read_fd_==-1) {
        ...
        do {
            ...
            iret = open_read_file();
            if (iret!=TFS_SUCCESS) {
                TBSYS_LOG(WARN, "reopen file(%s:%d) failed", queue_path_.c_str(),
...
int FileQueue::write_header()
{
    ...
    if (iret==TFS_SUCCESS) {
        ...
        do {
            ...
            TBSYS_LOG(ERROR, "write queue infromation header failed, path(%s), errors(%s)", queue_path_.c_str(),
                    strerror(
                            errno));
            ::close(information_fd_);
            information_fd_ = -1;
