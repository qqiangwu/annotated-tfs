 ...
 *
 *
 * Version: $Id: adminserver.cpp 50 2010-11-17 02:04:35Z nayan@taobao.com $
 *
 * Authors:
...
namespace tfs {
    namespace adminserver {
        ...
        void AdminServer::modify_conf(string& index, int32_t type)
        {
            ...
            int ret = system(cmd);
            TBSYS_LOG(INFO, "%s ds index %s %s", (1==type) ? "add" : "delete", index.c_str(),
                    (-1==ret) ? "fail" : "success");
        }
        ...
        int AdminServer::get_param(string& index)
        {
            ...
            if (index.size())         // dataserver
            {
                ...
                if (is_old_) {
                    ...
                    string conf_file = param->script_.substr(conf_start,
                            (string::npos==conf_end) ? conf_end : conf_end-conf_start);
                ...
                TBSYS_LOG(INFO, "load dataserver %s, desc : %s, lock_file : %s, port : %d, script : %s, waittime: %d\n",
                        index.c_str(), param->description_.c_str(), param->lock_file_.c_str(), param->adr_.port_,
                        param->script_.c_str(),
        ...
        int AdminServer::start(bool run_now)
        {
            ...
            // start queue thread
            task_queue_thread_.setThreadParameter(CONFIG.get_int_value(CONFIG_ADMINSERVER, CONF_THREAD_COUNT, 1), this,
                    NULL);
